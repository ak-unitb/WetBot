<?php
	...
	if ($_GET['id'] && !$_POST['id']) { // edit a sensor
	...
	} else if (!empty($_POST['id'])) { // request of saving a sensor
		...
		// writing the changed sensor information into the config.file for the YÙn-Side
		if ($saved) {
			...
			if ($rs === false) {
				...
				$fileHandle = fopen('../../sensors.csv', 'w');

				$rs->data_seek(0);
				while ($row = $rs->fetch_assoc()) {
					fwrite($fileHandle, $row['id'].',');
					fwrite($fileHandle, $row['frequency_threeshold_too_dry'].',');
					fwrite($fileHandle, $row['frequency_threeshold_too_wet'].',');
					fwrite($fileHandle, $row['active']);
					fwrite($fileHandle, "\n");
				}
...
?>
	...
	<p class="text-message bg-hint">Um Ihre Änderungen anzuwenden, müssen Sie die Sensor-Steuerung <a href="./index.php?reset-mcu=true" class="btn btn-default">neu starten</a>!</p>
<?php
	} else {
?>
